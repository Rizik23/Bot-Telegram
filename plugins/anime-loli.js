...
{"url":"https://i.pinimg.com/originals/cd/ef/fc/cdeffc0bf155fe2c8c63561b437c6864.jpg"}
  ...
  bot.command('loli', async (ctx) => {
    ...
    await ctx.replyWithPhoto({ url: pick.url }, {
      caption: '✨ Loli detected~ Siap protek! 🛡️',
      reply_markup: {
        inline_keyboard: [[{ text: '➡️ Next Loli', callback_data: 'loli_next' }]]
      }
    });
  });

  bot.action('loli_next', async (ctx) => {
      const getRandom = () => list[Math.floor(Math.random() * list.length)];
    ...
    try {
      ...
      const pick = getRandom();
      await ctx.replyWithPhoto({ url: pick.url }, {
        caption: '✨ Protek protek~ Loli baru siap!',
        reply_markup: {
          inline_keyboard: [[{ text: '➡️ Next Loli', callback_data: 'loli_next' }]]
        }
      });
