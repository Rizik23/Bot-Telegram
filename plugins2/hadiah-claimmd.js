...
module.exports = (bot) => {
  bot.command('claimmd', async (ctx) => {
    ...
    // Cek apakah user sudah klaim Script Md
    let notifLog = [];
    try {
    ...
    const sudahClaim = notifLog.some(n => n.userId === userId && n.claim === 'Script Md');
    if (sudahClaim) {
      return ctx.reply('⚠️ Kamu sudah pernah claim Script Md sebelumnya.');
    }
    ...
    if (saldoUser >= 200000) {
      ...
      return ctx.reply(
        `🎉 Kamu berhasil claim Script Md!\n\n💰 Sisa saldo kamu: Rp ${userData.money.toLocaleString()}`,
        Markup.inlineKeyboard([
          [Markup.button.url('📦 Download', 'https://www.mediafire.com/file/9ejyygkr4es3oof/Meliodas+V1.0+NOENCRIPT.zip/file')],
        ])
      );
    } else {
      return ctx.reply(
        `❌ Saldo kamu tidak cukup untuk claim Script Md.\n💰 Saldo kamu: Rp ${saldoUser.toLocaleString()}\n\n` +
