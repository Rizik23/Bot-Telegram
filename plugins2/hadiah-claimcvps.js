...
module.exports = (bot) => {
  bot.command('claimcvps', async (ctx) => {
    ...
    // Cek apakah user sudah pernah klaim Script Cvps
    let notifLog = [];
    try {
    ...
    const sudahClaim = notifLog.some(n => n.userId === userId && n.claim === 'Script Cvps');
    if (sudahClaim) {
      return ctx.reply('⚠️ Kamu sudah pernah claim Script Cvps sebelumnya.');
    }

    if (saldoUser >= 350000) {
      // Kurangi saldo dan update data
      userData.money -= 350000;
      ...
      return ctx.reply(
        `🎉 Kamu berhasil claim Script Cvps!\n\n💰 Sisa saldo kamu: Rp ${userData.money.toLocaleString()}`,
        Markup.inlineKeyboard([
          [Markup.button.url('📦 Download', 'https://www.mediafire.com/file/hp4fbk1uv3rv8kr/Cvps-[Telegram].zip/file')],
        ])
      );
    } else {
      return ctx.reply(
        `❌ Saldo kamu tidak cukup untuk claim Script Cvps.\n💰 Saldo kamu: Rp ${saldoUser.toLocaleString()}\n\n` +
