...
module.exports = (bot) => {
  ...
  bot.action(/^doaharian_(\d+)$/, async (ctx) => {
    const index = parseInt(ctx.match[1])
    await ctx.answerCbQuery().catch(() => {})
    await sendDoa(ctx, index, true)
  })
}

async function sendDoa(ctx, index, edit = false) {
  const item = doaList[index]
  if (!item) return ctx.reply('❌ Doa tidak ditemukan.')

  const total = doaList.length
  const text = `
...
${item.translation}
  ...
  const buttons = []
  if (index > 0) buttons.push(Markup.button.callback('⬅️ Sebelumnya', `doaharian_${index - 1}`))
  if (index < total - 1) buttons.push(Markup.button.callback('Selanjutnya ➡️', `doaharian_${index + 1}`))
