...
module.exports = (bot) => {
  ...
  function getRandomWaifu() {
    ...
    const pick = getRandomWaifu();

    await ctx.replyWithPhoto({ url: pick.url }, {
      caption: '💘 Waifu hari ini datang buat nemenin kamu~',
      reply_markup: {
        inline_keyboard: [
          [{ text: '➡️ Next Waifu', callback_data: 'waifu_next' }]
        ]
      }
    });
  });

  bot.action('waifu_next', async (ctx) => {
      function getRandomWaifu() {
    ...
    try {
      await ctx.answerCbQuery(); // ilangin loading
      await ctx.deleteMessage(); // hapus foto lama

      const pick = getRandomWaifu();

      await ctx.replyWithPhoto({ url: pick.url }, {
        caption: '💘 Waifu lain udah nongol buat kamu~',
        reply_markup: {
          inline_keyboard: [
            [{ text: '➡️ Next Waifu', callback_data: 'waifu_next' }]
          ]
      ...
      console.error('[WAIFU NEXT ERROR]', err.message);
      ctx.reply('❌ Gagal ganti waifu. Coba lagi.');
    }
  });
};
