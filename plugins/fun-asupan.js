...
module.exports = (bot) => {
  bot.command("asupan", async (ctx) => {
    // Opsional: Check premium user (bisa ganti dengan logika lo sendiri)
    const userId = String(ctx.from.id);
if (!isUserPremium(userId)) {

      return ctx.reply("❌ Fitur ini hanya untuk user *Premium*.\nBeli Akses Prem ke @VellzXyrine dulu yaaa.");

    }
    const videoUrl = pickRandom(asupan);

    try {
      await ctx.replyWithVideo({ url: videoUrl }, { caption: "_Nih kak_" });
    } catch (err) {
      console.error("❌ Error kirim asupan:", err.message);
      ctx.reply("❌ Gagal mengirim video asupan.");
    }
  });
};
